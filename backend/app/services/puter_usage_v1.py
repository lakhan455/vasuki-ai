    ...
    try:
        async with httpx.AsyncClient(timeout=8.0) as client:
            response = await client.post(
                f"{base}/rest/v1/rpc/consume_puter_image_quota",
...
async def consume_puter_image_quota(
    ...
    persistent = await _persistent_quota(
        user_id,
        daily_limit,
        settings,
    )
    ...
    return await _fallback_quota(user_id, daily_limit)
